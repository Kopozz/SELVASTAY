...
export default function ClientesPage() {
  ...
  const handleUpdateCliente = async (formData) => {
    ...
    try {
      ...
      toast.success(`${getClientName()} actualizado correctamente`);
      addAuditLog('CLIENTE_ACTUALIZADO', `${getClientName()} '${formData.nombre_completo}' editado con éxito.`);
    } catch (err) {
      toast.error(`No se pudo actualizar el ${getClientName().toLowerCase()}`);
      throw err;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-v-white)] tracking-tight">{getClientName(true)}</h1>
          <p className="text-[var(--color-v-gray-400)] text-sm mt-1">Directorio de {getClientName(true).toLowerCase()} registrados.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div key={c.id} className="v-card p-5 group">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-bold text-[var(--color-v-gray-500)] uppercase tracking-widest">{c.tipo_doc || 'DOCUMENTO'}</span>
              ...
              <div className="mt-5 pt-4 border-t border-[rgba(255,255,255,0.04)] flex justify-between items-center">
                <button 
                  ...
                  className="text-[var(--color-v-gray-400)] hover:text-[var(--color-v-white)] text-xs flex items-center gap-1 transition-colors cursor-pointer"
                >
                  <Edit2 size={12} /> Editar
                </button>
                <button className="text-[var(--color-v-green)] text-xs font-medium hover:underline flex items-center gap-1 cursor-pointer">
          ...
          {filtered.length === 0 && (
            <div className="col-span-full p-12 v-card text-center text-[var(--color-v-gray-500)] italic">
              No se encontraron {getClientName(true).toLowerCase()} que coincidan con la búsqueda.
            </div>
          )}
        </div>
      )}
